...
def warmup_model(model, data_loader, device, iterations=5):
...
def evaluate_informer_baseline(model, test_loader, device):
    ...
    return mse, inf_time, num_params
...
def train(sweep_config, args, device):
    ...
    informer_mse, informer_inf_time, informer_params = evaluate_informer_baseline(informer_model, test_loader, device)
    wandb.log({"informer_mse": informer_mse, "informer_inference_time": informer_inf_time, "informer_params": informer_params})
